...
public class LossCalculatorPositive {
				...
				if (totalPatchNumber !=1){
					for(int h=0;h<totalPatchNumber;h++){
						...
						for (File fImage: imageList){
							...
							String positive_ETLossPath =  root + "ETLoss_ratio/"+ imClass + "/"+String.valueOf(totalPatchNumber)+"/"+ETLossFileName;

							//File negative_f = new File(negative_ETLossPath);
							//negative_f.getAbsoluteFile().getParentFile().mkdirs();
							
							File positive_f = new File(positive_ETLossPath);
							positive_f.getAbsoluteFile().getParentFile().mkdirs();
							
//							String negative_ETGazesPath = root + "gaze_negative/" + objectClass+"/"+imageFileName[imageFileName.length - 1].replace(".jpg", ".txt") ; 
							String positive_ETGazesPath = root + "gazes/" +imageFileName[imageFileName.length - 1].replace(".jpg", ".txt") ; 
							
//							if(!negative_f.exists()) { 	
							if(!positive_f.exists()) {
								double positive_ETLoss = getETRatioLoss(imagePath, totalPatchNumber, h, positive_ETGazesPath);
								try {					
//									BufferedWriter out = new BufferedWriter(new FileWriter(negative_f));
									BufferedWriter out = new BufferedWriter(new FileWriter(positive_f));
									out.write(String.valueOf(positive_ETLoss));
									out.flush();
									out.close();
								} catch (IOException e) {
								e.printStackTrace();
