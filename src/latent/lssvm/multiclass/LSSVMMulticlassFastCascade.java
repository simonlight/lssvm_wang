...
public abstract class LSSVMMulticlassFastCascade<X,H> implements LatentStructuralClassifier<X,Integer,H> {
	...
	protected abstract Object[] lossAugmentedInference2(STrainingSample<LatentRepresentation<X, H>, Integer> ts);
	protected abstract H prediction2(X x);
	protected abstract double delta2(Integer yi, Integer yp, X x, H h);
	...
	public Object[] cuttingPlane(List<STrainingSample<LatentRepresentation<X,H>,Integer>> l) {
		...
		for(int i=0; i<l.size(); i++){
			STrainingSample<LatentRepresentation<X,H>,Integer> ts = l.get(i);			
			Object[] or = lossAugmentedInference2(ts);
			Integer yp = (Integer)or[0];
			H hp = (H)or[1];
			ct += delta2(ts.output, yp, ts.input.x, hp);
			double[] psi3 = psi(ts.input.x, hp);
			double[] psi4 = psi(ts.input.x, prediction2(ts.input.x));
//			double[] psi4 = psi(ts.input.x, ts.input.h);
			for(int d=0; d<w[ts.output].length; d++) {
//				gt[ts.output][d] 	-= psi3[d];
				gt[yp][d] 	-= psi3[d];
				gt[ts.output][d] 	+= psi4[d];
			}
		}
...
//			System.out.println("--------------");
			loss += delta2(ts.output, yp, ts.input.x, hp);
//			System.out.println(delta2(ts.output, yp, ts.input.x, hp));
