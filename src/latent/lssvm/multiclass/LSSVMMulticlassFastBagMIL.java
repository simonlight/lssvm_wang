...
public class LSSVMMulticlassFastBagMIL extends LSSVMMulticlassFast<BagMIL,Integer> {
	...
	public double testAPRegion(List<STrainingSample<LatentRepresentation<BagMIL, Integer>, Integer>> l, double epsilon, double lambda,int scale, String simDir, String className,String detailFolder) {
		
		List<Evaluation<Integer>> eval = new ArrayList<Evaluation<Integer>>();
		File resFile=new File(simDir+detailFolder+"/metric_"+String.valueOf(scale)+"_"+Double.valueOf(epsilon)+"_"+Double.valueOf(lambda)+"_"+className+".txt");
