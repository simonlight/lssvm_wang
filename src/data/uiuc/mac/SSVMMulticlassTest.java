...
import ssvm.multiclass.SSVMMulticlass;
import struct.STrainingSample;
import data.io.BagReader;
import fr.lip6.jkernelmachines.type.TrainingSample;

public class SSVMMulticlassTest {
    		...
    		for(int split : splitCV) {
    			...
    			for(double epsilon : epsilonCV) {
    		    	for(double lambda : lambdaCV) {
    		    		...
    		    		SSVMMulticlass svm = new SSVMMulticlass(); 
				...
				if(compute) {
	    			...
	    			for(double epsilon : epsilonCV) {
	    		    	for(double lambda : lambdaCV) {
	    		    		...
	    		    		//SSVMMulticlassFast svm = new SSVMMulticlassFast();
	    		    		SSVMMulticlass svm = new SSVMMulticlass();
							svm.setLambda(lambda);
							svm.setOptim(optim);
							svm.setCpmax(cpmax);
			    			...
			    			if(compute || !testFile) {
								...
								double acc = svm.evaluation(exampleTrain);
								System.err.println("train - " + cls + "\tscale= " + scale + "\tacc= " + acc + "\tlambda= " + lambda + "\tepsilon= " + epsilon);
								
								acc = svm.evaluation(exampleTest);
								svm.save(new File(classifierDir + "/" + cls + "/" + cls + "_" + scale + suffix + "_acc_" + acc + ".txt"));
